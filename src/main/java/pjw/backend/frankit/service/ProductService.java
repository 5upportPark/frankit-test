...
public class ProductService {
    ...
    public List<ProductView> getProductList(Long cursor){
        ...
        List<ProductView> list = result.stream().map(ProductView::from).toList();
        return list;
    }
    ...
    public ProductView updateProduct(ProductRequest req){
        if(req.getId() == null || req.getId().equals(0L)) return null;
        Product product = productRepositoryImpl.findById(req.getId()).orElseThrow(RuntimeException::new);
